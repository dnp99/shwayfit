...
Authenticated trainers can book appointments only for active clients with an active package that has at least one remaining session. A booking stores the client, start timestamp, duration, and optional notes; it does not reserve or reduce the package balance. The calendar loads the authenticated organization's appointments for the visible period and joins them with the authorized client directory for display.
