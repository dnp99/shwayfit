...
The first package slice provides trainer-managed options at `/packages`. Active organization members can create options with a name and one to one hundred included sessions, list active and archived options, and archive an option. The package page presents the reusable-template explanation, active and archived filters, and responsive option cards. When no active options exist, the screen prompts the trainer to create their first package in a reusable dialog. The API is organization-scoped and requires an active Firebase-backed membership.
