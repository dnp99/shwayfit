...
import { browserLocalPersistence, browserSessionPersistence, getAuth, setPersistence, signOut } from 'firebase/auth'
...
export function sessionExpiresAt() {
  const expiry = Number(sessionStorageFor(isTrustedDevice()).getItem(sessionExpiryKey))
...
export function beginFirebaseAuthSession() {
  ...
  sessionStorageFor(isTrustedDevice()).setItem(sessionExpiryKey, String(Date.now() + maxSessionAgeMs))
}
