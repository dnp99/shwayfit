...
function ClientDetailPage({ client, onBack, onBook, onEdit }: { client: Client; onBack: () => void; onBook: () => void; onEdit: () => void }) {
  ...
  return <section aria-labelledby="client-detail-heading">
    ...
    <div className="mt-8 grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
      ...
      <aside className="grid gap-5"><ClientPackagePanel clientID={client.id} clientName={`${client.firstName} ${client.lastName}`} /><section className="rounded-xl border border-border bg-card p-5 sm:p-6" aria-labelledby="contact-details-heading"><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-semibold tracking-tight" id="contact-details-heading">Contact details</h2><Button onClick={onEdit} size="sm" type="button" variant="outline">Edit</Button></div><dl className="mt-5 grid gap-4 text-sm"><div className="flex items-start gap-3"><Mail aria-hidden="true" className="mt-0.5 size-4 text-muted-foreground" /><div><dt className="text-muted-foreground">Email</dt><dd className="mt-1 font-medium">{client.email || 'Not recorded'}</dd></div></div><div className="flex items-start gap-3"><Phone aria-hidden="true" className="mt-0.5 size-4 text-muted-foreground" /><div><dt className="text-muted-foreground">Phone</dt><dd className="mt-1 font-medium">{formatPhone(client.phone) || 'Not recorded'}</dd></div></div>{client.preferredStartTime && client.preferredEndTime && <div><dt className="text-muted-foreground">Preferred time</dt><dd className="mt-1 font-medium">{client.preferredStartTime}–{client.preferredEndTime}</dd></div>}</dl></section></aside>
    </div>
  </section>
}
