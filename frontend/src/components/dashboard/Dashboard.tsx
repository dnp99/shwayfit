...
export function Dashboard({ trainerName }: { trainerName: string }) {
  ...
  return <section aria-labelledby="dashboard-heading">
    <header><p className="text-sm font-medium text-primary">{dateFormatter.format(now)}</p><h1 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl" id="dashboard-heading">Good morning{trainerName ? `, ${trainerName.split(/\s+/)[0]}` : ''}</h1><p className="mt-3 text-base leading-7 text-muted-foreground">{todayAppointments.length === 0 ? 'Your schedule is clear today.' : `You have ${todayAppointments.length} session${todayAppointments.length === 1 ? '' : 's'} today.`}</p></header>
