...
export function ClientPackagePanel({ clientID, clientName }: { clientID: string; clientName: string }) {
  ...
  return <section className="mt-6 border-t border-border pt-6" aria-labelledby="client-package-heading">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold tracking-[0.175em] text-secondary-foreground">CLIENT PACKAGE</p><h2 id="client-package-heading" className="mt-1 text-base font-semibold">Session balance</h2><p className="mt-1 text-sm text-muted-foreground">Assign and review this client’s current session allowance.</p></div>{!activePackage && activeOptions.length > 0 && <AssignPackageDialog clientName={clientName} isOpen={isOpen} onOpenChange={setIsOpen} isSaving={isSaving} options={activeOptions} onSubmit={assignPackage} />}</div>
    <div className="mt-4">
