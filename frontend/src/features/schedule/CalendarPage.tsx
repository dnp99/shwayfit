...
type Appointment = { id: string; clientId: string; startAt: string; durationMinutes: number; notes?: string; status: 'scheduled' }
...
function TimeGrid({ appointments, clients, days }: { appointments: Appointment[]; clients: Client[]; days: Date[] }) {
  ...
  return <div aria-label="Calendar time grid" className="overflow-x-auto rounded-xl border border-border bg-card"><div className="min-w-[27.5rem]" style={{ minWidth: days.length === 1 ? '27.5rem' : '51.25rem' }}>
    ...
    <div className="grid" style={{ gridTemplateColumns }}><div className="relative bg-muted/20" style={{ height: hours.length * hourHeight }}>{hours.map((hour) => <span className="absolute left-0 w-full -translate-y-1/2 pr-3 text-right text-xs text-muted-foreground" key={hour} style={{ top: hour * hourHeight }}>{formatHour(hour)}</span>)}</div>
      {days.map((day) => <div className="relative border-l border-border" key={day.toISOString()} style={{ height: hours.length * hourHeight }}><p className="sr-only">{todayFormatter.format(day)}</p>{hours.map((hour) => <span aria-hidden="true" className="absolute left-0 w-full border-t border-border/80" key={hour} style={{ top: hour * hourHeight }} />)}{appointments.filter((appointment) => isSameDay(new Date(appointment.startAt), day)).map((appointment) => { const start = new Date(appointment.startAt); const minutes = start.getHours() * 60 + start.getMinutes(); return <article className="absolute inset-x-1 z-10 overflow-hidden rounded-md border-l-4 border-primary bg-accent px-2 py-1.5 text-xs shadow-sm" key={appointment.id} style={{ top: Math.max(4, minutes / 60 * hourHeight + 4), height: Math.max(44, appointment.durationMinutes / 60 * hourHeight - 6) }}><p className="font-semibold">{timeFormatter.format(start)}</p><p className="mt-0.5 truncate font-medium">{clientName(appointment.clientId, clients)}</p><p className="mt-0.5 truncate text-muted-foreground">{appointment.durationMinutes} min</p></article> })}</div>)}</div>
  </div></div>
}
...
function BookingDialog({ appointments, clients, onBooked, onOpenChange, open }: { appointments: Appointment[]; clients: Client[]; onBooked: (appointment: Appointment) => void; onOpenChange: (open: boolean) => void; open: boolean }) {
...
export function CalendarPage() {
  ...
  const [error, setError] = useState<string | null>(null)
  const [isBooking, setIsBooking] = useState(() => searchParams.get('book') === '1')
  ...
  function setBooking(open: boolean) { setIsBooking(open); navigate(open ? '/schedule?book=1' : '/schedule', { replace: true }) }

  return <section aria-labelledby="calendar-heading"><header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-medium text-primary">Trainer schedule</p><h1 className="mt-1 text-3xl font-semibold tracking-tight sm:text-4xl" id="calendar-heading">Calendar</h1><p className="mt-2 text-muted-foreground">Manage training sessions across your schedule.</p></div><Button onClick={() => setBooking(true)} type="button"><Plus aria-hidden="true" className="size-4" />Book appointment</Button></header><div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><CalendarViewToggle onChange={setView} value={view} /><div aria-label="Calendar date controls" className="flex items-center gap-2"><Button aria-label="Previous period" onClick={() => movePeriod(-1)} size="icon" type="button" variant="outline"><ChevronLeft aria-hidden="true" className="size-4" /></Button><p aria-live="polite" className="min-w-36 text-center text-sm font-medium">{periodLabel(view, anchorDate, days)}</p><Button aria-label="Next period" onClick={() => movePeriod(1)} size="icon" type="button" variant="outline"><ChevronRight aria-hidden="true" className="size-4" /></Button><Button className="hidden sm:inline-flex" onClick={() => setAnchorDate(atStartOfDay(new Date()))} size="sm" type="button" variant="outline">Go to current</Button></div><Button className="sm:hidden" onClick={() => setAnchorDate(atStartOfDay(new Date()))} type="button" variant="outline">Go to current</Button></div><div className="mt-5 flex items-center gap-2 rounded-lg border border-info/30 bg-info/10 px-3 py-2.5 text-sm text-foreground"><Info aria-hidden="true" className="size-4 shrink-0 text-info" /><p><span className="font-medium">Overlaps are allowed.</span> ShwayFit warns you before booking a conflicting time.</p></div>{error && <p className="mt-4 text-sm text-destructive" role="alert">{error}</p>}<div className="mt-5">{view === 'month' ? <MonthGrid appointments={appointments} clients={clients} date={anchorDate} onSelectDay={selectMonthDay} /> : <TimeGrid appointments={appointments} clients={clients} days={days} />}</div>{appointments.length === 0 && <section aria-label="Calendar availability" className="mt-5 grid place-items-center rounded-xl border border-dashed border-border bg-card/50 px-6 py-8 text-center"><div><span className="mx-auto grid size-11 place-items-center rounded-full bg-secondary text-secondary-foreground"><CalendarDays aria-hidden="true" className="size-5" /></span><h2 className="mt-4 font-semibold">No appointments scheduled</h2><p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">Book a session to begin building this schedule.</p></div></section>}<BookingDialog appointments={appointments} clients={clients} onBooked={(appointment) => setAppointments((current) => [...current, appointment])} onOpenChange={setBooking} open={isBooking} /></section>
}
