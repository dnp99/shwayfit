...
import { DesktopSidebar, MobileBottomNav, MobileHeader } from './AppNavigation'
...
function ApplicationShell({ user }: { user: User }) {
  ...
  return <main className="min-h-screen bg-background text-foreground lg:flex"><DesktopSidebar email={user.email ?? ''} /><div className="min-w-0 flex-1"><MobileHeader /><div className="mx-auto w-full max-w-7xl px-4 py-6 pb-24 sm:px-6 sm:py-8 lg:px-10 lg:py-10 lg:pb-10">{content}</div></div><MobileBottomNav /></main>
}
