...
function ApplicationShell({ user }: { user: User }) {
  return <main className="min-h-screen bg-background text-foreground"><header className="border-b border-border bg-card"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6"><a className="inline-flex items-center gap-2 text-lg font-semibold tracking-tight" href="/home"><span className="grid size-8 place-items-center rounded-lg bg-primary text-brand-lime" aria-hidden="true">s</span>ShwayFit</a><ThemeToggle /></div><nav className="mx-auto grid max-w-7xl grid-cols-5 gap-1 px-3 pb-3 sm:px-6" aria-label="Product navigation">{areas.map(([to, label]) => <NavLink className={({ isActive }) => `flex min-h-11 items-center justify-center rounded-lg px-2 text-center text-xs font-medium transition-colors ${isActive ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`} key={to} to={to}>{label}</NavLink>)}</nav></header><div className="px-5 py-6 sm:px-10 lg:px-20"><Routes><Route path="/clients" element={<TrainerWorkspace email={user.email ?? ''} />} /><Route path="/home" element={<FutureArea title="Home" description="Your trainer overview will bring upcoming sessions, package balances, and quick actions together after packages and scheduling are in place." />} /><Route path="/packages" element={<FutureArea title="Packages" description="Package options and client balances are the next product slice." />} /><Route path="/schedule" element={<FutureArea title="Schedule" description="Appointments will be added once client packages provide the balance and audit foundation." />} /><Route path="/settings" element={<FutureArea title="Settings" description="Trainer and organization preferences will follow the core workflow." />} /><Route path="*" element={<Navigate to="/clients" replace />} /></Routes></div></main>
}
